...
$core_options = new ZuraMetaCoreControl();

function zura_options($params = array())
{
    ...
    if (is_admin() && !empty($params['id']) && isset($core_options)) {
        ...
        elseif ($pagenow=='post-new.php' || $pagenow=='post.php'){
            ...
            // Render params id
            $params['id'] = "_zura_".$params['id'];
            // Get value
            if(isset($zura_meta->$params['id'])){
                $params['value'] = $zura_meta->$params['id'];
            } else {
                $params['value'] = null;
            }
        ...
        esc_html_e('Error', 'zura');
    }
    wp_enqueue_script('core-options');
}

class ZuraMetaCoreControl
{
    ...
    function Scripts()
    {
        ...
        wp_register_style('jquery-datetimepicker', get_template_directory_uri().'/includes/libs/metacore/assets/css/jquery.datetimepicker.css');
        wp_register_style('jquery-minicolors', get_template_directory_uri() . '/includes/libs/metacore/assets/css/jquery.minicolors.css');
        wp_register_style('core-options', get_template_directory_uri() . '/includes/libs/metacore/assets/css/core.options.css');
        wp_register_style('jquery.nouislider', get_template_directory_uri() . '/includes/libs/metacore/assets/css/jquery.nouislider.css');
        wp_register_style('jquery.nouislider.pips', get_template_directory_uri() . '/includes/libs/metacore/assets/css/jquery.nouislider.pips.css');

        wp_register_script('jquery-datetimepicker', get_template_directory_uri() . '/includes/libs/metacore/assets/js/jquery.datetimepicker.js');
        wp_register_script('jquery-minicolors', get_template_directory_uri() . '/includes/libs/metacore/assets/js/jquery.minicolors.js');
        wp_register_script('media-selector', get_template_directory_uri() . '/includes/libs/metacore/assets/js/media.selector.js');
        wp_register_script('jquery.nouislider', get_template_directory_uri() . '/includes/libs/metacore/assets/js/jquery.nouislider.all.js');
        wp_register_script('icons-class', get_template_directory_uri() . '/includes/libs/metacore/assets/js/icons.class.js');
        ...
        wp_register_script('core-options', get_template_directory_uri() . '/includes/libs/metacore/assets/js/core.options.js');

        global $pagenow;
        if (is_admin() && ($pagenow == 'post-new.php' || $pagenow == 'post.php')) {
            wp_enqueue_style('metabox', get_template_directory_uri() . '/includes/libs/metacore/assets/css/metabox.css');
            wp_enqueue_script('easytabs', get_template_directory_uri() . '/includes/libs/metacore/assets/js/jquery.easytabs.min.js');
            wp_enqueue_script('metabox', get_template_directory_uri() . '/includes/libs/metacore/assets/js/metabox.js');
        }
    }
        ...
        <div class="images-field clearfix">
            ...
            foreach ($selected as $value):
                ...
                <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]);?>);background-size:cover;">
                    <i class="edit dashicons dashicons-plus-alt" title="<?php esc_html_e('Replace Image', 'zura'); ?>"></i>
                    <i class="remove dashicons dashicons-dismiss" title="<?php esc_html_e('Remove Image', 'zura'); ?>"></i>
                </li>
            <?php endif; endforeach; ?>
            <?php if($params['field'] != 'single'): ?>
                <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php esc_html_e('Add Image', 'zura'); ?>"></i></li>
    ...
    private function file($params){
        ...
        <div class="file-field csfield">
    		...
    		<input class="zura_upload_button button button-primary" type="button" value="Browse" />
    		<input data-id="<?php echo esc_attr($params['id']); ?>" class="zura_clear_button button button-danger" type="button" value="Clear" />
        </div>
        <?php
        return ob_get_clean();
    }
    ...
    private function social($params){
		...
		$zura_social_team = json_decode($content);
		wp_enqueue_style('css.ui');
		wp_enqueue_script('jquery.ui.min');
        ob_start();
        ?>
		<table class="wp-list-table widefat fixed pages zura_social_team">
			<thead>
				<tr>
					<th><?php esc_html_e('Title', 'zura');?></th>
					<th><?php esc_html_e('Link', 'zura');?></th>
					<th><?php esc_html_e('Icon class', 'zura');?></th>
					<th><?php esc_html_e('Actions', 'zura');?></th>
				</tr>
			</thead>
			<tfoot>
				<tr>
				...
				<tr>
					<td colspan="3">
						<a class="button-primary" id="btn_add_social" href="javascript:void(0);"><?php esc_html_e('Add new', 'zura');?></a>
					</td>
				</tr>
			</tfoot>
			<tbody  id="the-list">
				<?php
					if(count($zura_social_team)):
						foreach($zura_social_team as $k=>$v): ?>
						<tr>
							...
							<td><a title="Click to edit this item" href="javascript:void(0)" class="zura_social"><?php esc_html_e('Edit', 'zura')?></a>|<a class="zura_social_remove" href="javascript:void(0)"><?php esc_html_e('Remove', 'zura')?></a></td>
						</tr>
						<?php endforeach;
					else: ?>
						<tr>
							<td class="zura_no_item" colspan="3"><?php esc_html_e('No item!', 'zura');?></td>
						</tr>
					<?php endif;
				?>
			</tbody>
				...
				<table class="widefat fixed pages tb_woo_countries">
					<thead>
						<tr>
							<th><?php esc_html_e('Title', 'zura');?></th>
							<th><?php esc_html_e('Link', 'zura');?></th>
							<th><?php esc_html_e('Icon', 'zura');?></th>
						</tr>
					</thead>
					<tbody>
						<tr>
        ...
        <div class="social-field csfield">
            <input type="hidden" name="_zura_socials" id="_zura_socials" class="xvalue" value="<?php echo esc_attr($content); ?>"/>
        </div>
        <?php
        return ob_get_clean();
    }
    ...
    public function metabox($params){
        ob_start();
        ?>
		<div id="zura_metabox_field_<?php echo esc_attr($params['id']); ?>" class="zura_metabox_field clearfix">
    ...
    public function save_meta_boxes($post_id)
	{
		...
		foreach($_POST as $key => $value) {
			if(strstr($key, '_zura_')) {
			    $zura_meta[$key] = $value;
			}
		}
		/* update _zura_meta_data. */
        if(!empty($zura_meta)){
		    update_post_meta($post_id, '_zura_meta_data', str_replace("\\\'","\\'",json_encode($zura_meta)));
		}
	}
}